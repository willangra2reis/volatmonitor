...
from google_auth import check_user_status, get_user_first_name
...
<body class="dark-mode">
        ...
        <div class="modal-content">
            ...
            <div class="chart-sidebar">
                ...
                <div class="chart-modal-header">
                    ...
                    <h3><span data-i18n="technical_analysis">Volat Scalper Pro</span></h3>
                ...
                <div class="chart-toolbar">
                    <div class="chart-toolbar-group">
                        ...
                        <label><input type="checkbox" id="ind-hull" checked> <span data-i18n="hull_ma">Hull MA</span></label>
                        <div class="slider-container">
                            <input type="range" id="hull-period" min="5" max="250" value="20" class="period-slider">
                            <span class="slider-value" id="hull-period-value">20</span>
                        </div>
                    </div>
                    
                    <div class="chart-toolbar-group">
                        <label data-i18n="oscillators">Osciladores:</label>
                        <label><input type="checkbox" id="ind-zscore" checked> <span data-i18n="zscore">Z-Score</span></label>
                        <div class="slider-container">
                            <input type="range" id="zscore-period" min="5" max="90" value="20" class="period-slider">
                            <span class="slider-value" id="zscore-period-value">20</span>
                        </div>
            ...
            <div class="chart-main-area">
                ...
                <div class="chart-info-panel">
                    ...
                    <span><span data-i18n="zscore_value">Z-Score</span>: <strong id="chart-zscore-value">-</strong></span>
                    ...
                    <div class="card h-100 shadow-sm border-start border-primary border-4">
                        <div class="card-body py-3">
                            <div class="d-flex justify-content-between align-items-center">
                                <div>
                                    <h6 class="card-subtitle text-muted dark-mode-text" data-i18n="current_balance">Saldo Atual</h6>
                    ...
                    <div class="card h-100 shadow-sm border-start border-info border-4">
                        <div class="card-body py-3">
                            <div class="d-flex justify-content-between align-items-center">
                                <div>
                                    <div class="d-flex justify-content-between align-items-center">
                                        <h6 class="card-subtitle text-muted dark-mode-text mb-0" data-i18n="equity">Equity</h6>
                            ...
                            <div class="d-flex justify-content-between align-items-center">
                                <div>
                                    <div class="d-flex align-items-center mb-1">
                                        <h6 class="card-subtitle text-muted dark-mode-text mb-0" data-i18n="daily_pl">L/P do Dia</h6>
                    ...
                    <div class="card h-100 shadow-sm border-start border-secondary border-4">
                        <div class="card-body py-3">
                            <div class="d-flex justify-content-between align-items-center">
                                <div>
                                    <div class="d-flex justify-content-between align-items-center">
                                        <h6 class="card-subtitle text-muted dark-mode-text mb-0" data-i18n="trades_today">Trades Hoje</h6>
                                ...
                                <div class="row g-1">
                                    ...
                                    <div class="col-6">
                                        <button id="close-positive-btn" class="btn btn-success btn-sm w-100" title="Fechar apenas ordens positivas">
                                            <i class="fas fa-plus-circle me-1"></i><span data-i18n="close_positive">Fechar +</span>
                                        </button>
                                    </div>
    ...
    <script>
            ...
            priceChartAdvanced = new Chart(ctx, {
                ...
                options: {
                    ...
                    scales: {
                        ...
                        y: {
                            display: true,
                            title: { display: true, text: 'Preço', color: '#aaa' },
        ...
        function startChartUpdates() {
            ...
            }, 100);  // 0.5 segundos = atualização muito rápida
        }
            ...
            checkboxes.forEach(id => {
                ...
                if (checkbox) {
                    checkbox.addEventListener('change', function() {
                        ...
                        } else if (document.getElementById('advanced-chart-modal').style.display === 'block') {
                            initializeAdvancedCharts();
                        }
    ...
    <footer style="position: fixed; bottom: 0; left: 0; right: 0; background: rgba(20, 20, 20, 0.95); padding: 10px; text-align: center; border-top: 1px solid #333; z-index: 1000;">
        <div style="display: flex; justify-content: center; align-items: center; gap: 20px; font-size: 12px; color: #888;">
            <span>© 2024 VolatForex Monitor Pro</span>
            <span>|</span>
...
def get_indicators(symbol):
    ...
    try:
        ...
        hull_period = int(request.args.get('hull_period', 20))
        zscore_period = int(request.args.get('zscore_period', 20))
...
def login():
        ...
        try:
            ...
            if user_data:
                ...
                if user_data.get('status') == 'PURCHASE_APPROVED':
                    ...
                    # Se "lembrar login" estiver marcado, salva cookie
                    resp = make_response(redirect(url_for('dashboard')))
                    if remember_me:
                        resp.set_cookie('remember_login', 'true', max_age=30*24*60*60, httponly=True, secure=False)
                        resp.set_cookie('user_email', email, max_age=30*24*60*60, httponly=True, secure=False)
                        resp.set_cookie('session_token', session_token, max_age=30*24*60*60, httponly=True, secure=False)
    ...
    # Verifica se há cookie de "lembrar login"
    if request.cookies.get('remember_login') == 'true':
        saved_email = request.cookies.get('user_email')
        if saved_email:
            try:
                ...
                if user_data and user_data.get('status') == 'PURCHASE_APPROVED':
                    ...
                    session['user_data_timestamp'] = user_data.get('data', '')
                    session.permanent = True
                    print(f"[LOGIN] 🔄 Login automático: {saved_email} - Token: {session_token[:16]}...")
...
def logout():
    ...
    # Remove cookies de "lembrar login"
