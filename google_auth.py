...
GOOGLE_API_URL = "https://script.google.com/macros/s/AKfycbx21yHNlZvC53GQa304n_gEgWY1J6tyD7ElRJej1Fklx5aPxReqgp1EKID3mfTvcD2P/exec"
...
def get_user_messages(email: str) -> list:
    """
    Retorna as mensagens personalizadas do usuário
    ...
    if user_data:
        return user_data.get("mensagens", [])
    
    return []


def get_user_notifications(email: str) -> list:
    """
    ...
    Returns:
        Lista de notificações ou lista vazia
    """
    user_data = check_user_status(email)
    
    if user_data:
        return user_data.get("notification", [])
