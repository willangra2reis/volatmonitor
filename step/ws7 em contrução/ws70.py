...
<body>
    ...
    <script>
                ...
                .then(([data, balanceHistory]) => {
                    if (data && data.account_info) {
                        ...
                        if (changes.tradesChanged && data.last_trades && data.last_trades.length > 0) {
                            ...
                            updateTimeGroupedChart(data.last_trades);
                        ...
                        // Atualiza card de Trades no Dia
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            setInterval(updateData, 1500); // Atualização periódica

            document.getElementById('time-group-selector').addEventListener('change', () => {
                if (window.lastApiData) {
                    updateCharts(window.lastApiData);
                }
            });
        });
        
        function updateCharts(data) {
            window.lastApiData = data; // Armazena os dados para uso do seletor

            // Atualiza o gráfico de evolução do saldo
            updateEvolutionChart(data.balance_history, data.account_info);

            // Atualiza os gráficos baseados em trades
            updateProfitChart(data.last_trades);
            updateSymbolChart(data.last_trades);
            updateDistributionChart(data.last_trades);
            updateTimeGroupedChart(data.last_trades);
        }
    </script>
</body>
