...
<body>
        ...
        <main class="content">
                    ...
                    <div class="card shadow-sm border-light-subtle">
                        <div class="card-body">
                            <div class="row align-items-center">
                                <!-- Controles -->
                                <div class="col-lg-3">
                                    <h5 class="card-title mb-3"><i class="fas fa-bullseye me-2"></i>Metas e Limites Diários</h5>
                                ...
                                <div class="col-lg-9">
                                    <div class="w-100">
                                        <div class="d-flex justify-content-between small text-muted mb-1">
                                            <span id="loss-limit-text">Limite Perda: -$0.00</span>
                                            <span class="fw-bold" id="daily-progress-text">Progresso: $0.00</span>
                                            <span id="goal-target-text">Meta Ganho: +$0.00</span>
                                        </div>
                                        <div class="progress-dual">
            ...
            <div class="row">
                <div class="col-12">
                    <div class="card shadow-sm border-light-subtle">
                        <div class="card-body">
                            <h5 class="card-title"><i class="fas fa-history me-2 text-primary"></i>Últimos 30 Trades Fechados</h5>
                            <div class="table-responsive">
                                <table class="table table-hover table-sm small">
                                    ...
                                    <tbody id="trades-tbody">
                                        <tr><td colspan="7" class="text-center p-5">Aguardando dados...</td></tr>
                                    </tbody>
                                </table>
                            </div>
                        </div>
    ...
    <script>
        ...
        function updateDailyMetrics(accountInfo) {
            ...
            // Atualiza textos
            document.getElementById('goal-target-text').textContent = `Meta Ganho: ${formatCurrency(goalTarget)}`;
            document.getElementById('loss-limit-text').textContent = `Limite Perda: ${formatCurrency(lossLimitTarget)}`;
            document.getElementById('daily-progress-text').textContent = `Progresso: ${formatCurrency(dailyProfit)}`;
        ...
        function updateData() {
            ...
            // Busca dados atuais e histórico em paralelo
            Promise.all([
                fetch('/api/latest').then(r => r.json()),
                fetch('/api/balance-history').then(r => r.json())
            ])
                .then(([data, balanceHistory]) => {
                    if (data && data.account_info) {
                        const changes = hasDataChanged(data);
                        
                        // Só atualiza elementos que realmente mudaram
                        if (changes.dataChanged) {
                            // Atualiza informações da conta
                            document.querySelector('#balance').textContent = formatCurrency(data.account_info.account_balance);
                            document.querySelector('#equity').textContent = formatCurrency(data.account_info.account_equity);

                            // Atualiza as métricas diárias (metas e limites)
                            if (window.goalControlsInitialized) {
                                updateDailyMetrics(data.account_info);
                            }
                            

                            const profit = data.account_info.account_profit;
                            const profitElement = document.querySelector('.profit-card .card-title');
                            if (profitElement) {
                                profitElement.textContent = formatProfit(profit);
                                profitElement.classList.remove('text-success', 'text-danger');
                                profitElement.classList.add(profit >= 0 ? 'text-success' : 'text-danger');
                            }
                        }
                        
                        // Atualiza gráfico de evolução sempre que houver novos dados de conta
                        if (balanceHistory && balanceHistory.length > 0) {
                            createEvolutionChart(balanceHistory);
                        }
                        
                        // Só atualiza trades e gráficos se os dados de trade mudaram
                        if (changes.tradesChanged && data.last_trades && data.last_trades.length > 0) {
                            const tbody = document.getElementById('trades-tbody');
                            
                            // Preserva scroll antes de atualizar tabela
                            const scrollBefore = isUserInteracting ? window.scrollY : null;
                            
                            // Atualiza gráficos usando dados mais recentes
                            createProfitChart(data.last_trades);
                            createSymbolChart(data.last_trades);
                            
                            // Atualiza tabela apenas se necessário
                            tbody.innerHTML = '';
                            ...
                            // Restaura scroll se usuário estava interagindo
                            if (scrollBefore !== null) {
                                requestAnimationFrame(() => {
                                    window.scrollTo({ top: scrollBefore, behavior: 'instant' });
                                });
                            }
                        } else if (changes.tradesChanged && (!data.last_trades || data.last_trades.length === 0)) {
                            // Se não há trades, limpa a tabela
                            document.getElementById('trades-tbody').innerHTML = '<tr><td colspan="7" style="text-align: center;">Nenhum trade fechado encontrado</td></tr>';
                        }
                        
                        // Atualiza timestamp sempre
                        document.getElementById('last-update').textContent = 
                            'Última atualização: ' + moment(data.timestamp).format('DD/MM/YYYY HH:mm:ss');
        ...
        document.addEventListener('DOMContentLoaded', function () {
            const goalPercentInput = document.getElementById('goal-percent');
            const limitPercentInput = document.getElementById('limit-percent');
            const saveGoalsButton = document.getElementById('save-goals');
            const goalModal = document.getElementById('goal-modal');
            const lossLimitModal = document.getElementById('loss-limit-modal');
            const closeModalButton = goalModal.querySelector('.close-button');
            const lossCloseModalButton = lossLimitModal.querySelector('.close-button');

            // Carregar valores salvos
            goalPercentInput.value = localStorage.getItem('goalPercent') || '2';
            limitPercentInput.value = localStorage.getItem('limitPercent') || '1';

            saveGoalsButton.addEventListener('click', () => {
                localStorage.setItem('goalPercent', goalPercentInput.value);
                localStorage.setItem('limitPercent', limitPercentInput.value);
            ...
            window.goalControlsInitialized = true; // Sinaliza que os controles estão prontos

            updateData(); // Carga inicial
            setInterval(updateData, 2000); // Atualização periódica
        });
    </script>
</body>
</html>
